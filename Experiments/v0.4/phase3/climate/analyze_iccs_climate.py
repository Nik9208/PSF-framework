...
LAYERS = {
    "RAW": "climate_raw_processed.csv",
    "FT": "climate_ft_surrogate.csv",
    "IAAFT": "climate_iaaft_surrogate.csv"
}
...
def run_layer_analysis(layer_name, file_name):
...
if __name__ == "__main__":
    ...
    for layer, filename in LAYERS.items():
        profiles[layer] = run_layer_analysis(layer, filename)
        update_ledger(f"E001_CLIM_{layer}", f"ICCS executed successfully on frozen {layer} dataset.")
        
    # Surrogate Sensitivity Fingerprint Calculation
    # Note: TE vector values might be singular or arrays. In ICCS v0.3.1 TE is usually a single scalar.
    raw_te_fwd = profiles["RAW"].vector["TE_forward"]
    raw_te_rev = profiles["RAW"].vector["TE_reverse"]
    raw_te_asym = raw_te_rev - raw_te_fwd
    ...
    metadata = {
        "domain": "Climate (NOAA Niño 3.4 SST)",
        "iccs_version": "0.3.1",
    ...
    json_path = os.path.join(BASE_DIR, "analysis_metadata_climate.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(metadata, f, indent=4)
        
    update_ledger("E002_CLIM_FINGERPRINT", "Surrogate Sensitivity Fingerprint generated.")
    
    print("\n--- Surrogate Sensitivity Fingerprint ---")
    print(json.dumps(fingerprint, indent=4))
    print("\n[Phase 3.1B ICCS Execution Complete]")
